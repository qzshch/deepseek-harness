...
**欢迎声明复用既有持久化字段。** 完整文案与版本由 `onboarding-copy.ts` 持有。该步骤通过既有 settings API 比较和写入 `ui-onboarding.welcomeNoticeVersion`，且只有点击「继续」才确认当前版本；welcome namespace 无法读取时，该步骤停留在本地化错误提示上。[2026-08-24 的远程设置访问决策](../architecture/2026-08-24-remote-settings-access.md)使受信远程浏览器能触达该 settings namespace，因此不再存在进程内回退。不改变 Host schema、API Proxy 允许列表或持久化实现。
