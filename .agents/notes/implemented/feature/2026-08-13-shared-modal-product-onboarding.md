...
**The welcome notice reuses the existing durable field.** Its exact copy and version live in `onboarding-copy.ts`. The step compares and writes `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Continue acknowledges the current version; a welcome namespace that cannot be read leaves the step on a localized error line. [The 2026-08-24 remote-settings-access decision](../architecture/2026-08-24-remote-settings-access.md) made the settings namespace reachable from trusted remote browsers, so no process-local fallback remains. No Host schema, API-proxy allowlist, or persistence implementation changes.
