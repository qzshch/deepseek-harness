...
**Reading and writing configuration move together.** When this note shipped, that meant both joined the loopback-only set; [the 2026-08-24 remote-settings-access decision](2026-08-24-remote-settings-access.md) supersedes the pinning — the settings data and credential planes now ride the browser-trust fence like every other method. The model catalog (`llm.providers`, `llm.models`) stays deliberately open: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it.
