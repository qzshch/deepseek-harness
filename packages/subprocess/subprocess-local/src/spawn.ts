...
export function spawnSubprocess(spec: SubprocessSpawnSpec, internals: SpawnInternals = {}): LocalSubprocessHandle {
  ...
  const child = spawn(program, args, {
    ...
    // Windows: hide the child's console window (CREATE_NO_WINDOW). Without it,
    // a console program spawned by a console-less host (e.g. a detached DSH
    // server) gets a fresh visible console per spawn.
    windowsHide: true,
  })
