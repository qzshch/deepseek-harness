...
只有当宿主确认可准备好一份由提供方持有的本地文档时，外壳才渲染**打开配置文件**；每个浏览器都注册该操作，其可用性跟随 Host 的 `hasDocument` 应答。该操作会在原生文本编辑器中打开该文档（macOS 上绕过浏览器文件关联）。
