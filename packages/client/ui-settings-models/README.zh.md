...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。它通过 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。welcome namespace 无法读取（传输失败）时，该步骤停留在本地化错误提示上，Continue 保持不可用，直到读取再次成功。
