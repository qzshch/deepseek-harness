// A trusted non-loopback browser reaches the settings plane through the same
// /api fence as a loopback one: the welcome acknowledgement persists in the
// Host document and does not re-present after reload.
...
describe.skipIf(MODE === 'record')('web e2e: remote welcome notice', () => {
  ...
  it('persists the acknowledgement to the Host document across a reload', async () => {
    ...
    // The fence admitted the remote browser's write: the Host document holds
    // the exact acknowledgement, not a process-local flag.
